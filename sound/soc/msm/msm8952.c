...
static int msm_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	pr_err("%s------enter: substream = %s  stream = %d\n", __func__,
		 substream->name, substream->stream);
    ...
    pr_err("%s------leave: substream = %s  stream = %d\n", __func__,
		 substream->name, substream->stream);
	return ret;
}
...
{
	printk("create quin status proc file\n");
...
static void remove_quin_status_proc_file(void)
{
	...
	printk("remove quin status proc file\n");
...
/* ASUS_BSP mei_huang --- */
	...
	pr_err("%s:  before create_quin_status_proc_file\n",
				__func__);
	create_quin_status_proc_file();
	pr_err("%s:  after create_quin_status_proc_file\n",
				__func__);
	//Create the quin checking status proc file;asus_bsp--
	//ASUS_BSP +++ tyree_liu@asus.com  add for codec_status
       if(!codec_status){
