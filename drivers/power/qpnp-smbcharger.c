...
static void asus_charge_config_sr(struct smbchg_chip *chip){
	...
	if(asus_project_id==ASUS_ZC552KL){
		printk("[CHARGE]asus_charge_config_sr use ZC552KL config\n");
...
static void asus_soft_jeita_config_zc552kl(struct smbchg_chip *chip,bool *charging_enable,int *fcc_value, int *float_volt)
{
...
static void smbchg_pre_config_zc552kl(struct smbchg_chip *chip)
{
...
/*	rc = smbchg_sec_masked_write(chip, chip->misc_base + 0xF1,
	...
	if(asus_project_id==ASUS_ZC552KL){
		smbchg_pre_config_zc552kl(chip);
...
static int asus_update_all(struct smbchg_chip *chip,struct battery_info_reply *batt_info)
{
	...
	pr_smb_rt(PR_STATUS, "CHGR_STS = 0x%02x  AICL:%d\n", reg,smbchg_get_aicl_level_ma(chip));
	return 0;
}
