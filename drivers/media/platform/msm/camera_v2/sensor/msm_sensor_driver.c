...
//[BSP] Sheldon_Li add camera proc (pisces)++
         remove_proc_file();	
	remove_file();     
	remove_module_file(); 
...
//[BSP] Sheldon_Li add camera proc (pisces)++
	set_sensor_info(slave_info->camera_id,s_ctrl,slave_info->sensor_id_info.sensor_id);
	create_otp_proc_file(slave_info); //read otp
...
static int32_t msm_sensor_driver_platform_probe(struct platform_device *pdev)
{
	...
	if(ret)
		printk("[Camera] : error to gpio_request\n");

	ret = gpio_direction_output(46,1);
	if(ret)
		printk("[Camera] : error to gpio_direction_output\n");
